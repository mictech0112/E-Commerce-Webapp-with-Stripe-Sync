                ...
                <div class="p-6 bg-white border-b border-gray-200">
                  ...
                  <form method="post" action="{{ route('admin.images.update', ['image' => $image->id ] )}}" >
                    ...
                    <form id="delete_{{$image->id}}" method="post" action="{{ route('admin.images.destroy', ['image' => $image->id ] )}}">
                      @csrf
                      @method('delete')
                      <div class="p-2 w-full flex justify-center mt-32">
                        <a href="#" data-id="{{ $image->id }}" onclick="deletePost(this)" class="text-white bg-red-500 border-0 py-2 px-4 focus:outline-none rounded ">削除する</a>                        
                      </div>
                    </form>
                </div>
            </div>
